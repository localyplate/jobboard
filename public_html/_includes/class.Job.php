...
class Job {
    ...
    public function Search($keywords, $url_query, $start_page = 1) {
        ...
        if (SEARCH_METHOD == 'classic') {
            ...
            if (!$found_city) {
                ...
                if ($kw1 == '' && $kw2 == '') {
                    ...
                    $conditions = 'title LIKE "%' . $keywords . '%" OR company LIKE "%' . $keywords . '%"' . ' OR description LIKE "%' . $keywords . '%"' . ' OR poster_email LIKE "%' . $keywords . '%" OR outside_location LIKE "%' . $keywords . '%"' . $extra_conditions;
        ...
        if ($total_pages > 1) {
            ...
            elseif (($pagination_loop - 1) > 0)
                $pages .= "&nbsp;<a href='" . BASE_URL . "search/" . $url_query . "/?p=" . ($pagination_loop - 1) . "'>&laquo;</a>&nbsp;";
            ...
            while (($pagination_loop < ($total_pages + 1)) && ($pagination_loop < $pagination_top)) {
                if ($pagination_loop == $start_page)
                    $pages .= "&nbsp;<a class='current_page' href='" . BASE_URL . "search/" . $url_query . "/?p=$pagination_loop'>$pagination_loop</a>&nbsp;";
                else
                    $pages .= "&nbsp;<a href='" . BASE_URL . "search/" . $url_query . "/?p=$pagination_loop'>$pagination_loop</a>&nbsp;";
                $pagination_loop++;
            }

            if ($pagination_loop == $pagination_top)
                $pages .= "&nbsp;<a href='" . BASE_URL . "search/" . $url_query . "/?p=" . ($pagination_loop) . "'>&raquo;</a>&nbsp;";
        }
    ...
    public function Create($params) {
        ...
        $sql = 'INSERT INTO ' . DB_PREFIX . 'jobs (type_id, category_id, title, description, company, city_id, url, apply, created_on, is_temp, is_active, 
			                       views_count, auth, outside_location, poster_email, apply_online, spotlight, occupation, details, address, requ, wage)
								 VALUES (' . $params['type_id'] . ',
		                                 ...
		                                 , ' . $params['spotlight'] . ', "' . $params['occupation'] . '", "' . $params['details'] . '", "' . $params['address'] . '", "' . $params['requ'] . '", "' . $params['wage'] . '")';
        $result = $db->query($sql);
